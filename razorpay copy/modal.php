...
<body>
    <div class="wrap">
        <h2>Modal Checkout</h2>
        <div class="muted">This page emulates a true modal checkout using the JS handler and posts the result to verify.php.</div>

        <div class="meta">
            <div><strong>Transaction ID:</strong> <?php echo h((string)$transaction_id); ?></div>
            <div><strong>Razorpay Order ID:</strong> <?php echo h($razorpay_order_id); ?></div>
            <div><strong>Amount (INR):</strong> <?php echo h($amount_inr); ?></div>
        </div>

        <button id="payBtn" type="button">Pay Now (Modal)</button>
        <div class="meta"><a href="<?php echo h($back_url); ?>">Back</a></div>
    </div>
    ...
    <script>
        (function () {
            ...
            options.handler = function (response) {
                ...
                form.method = 'POST';
                form.action = <?php echo json_encode($verify_url, JSON_UNESCAPED_SLASHES); ?>;
            ...
            options.modal.ondismiss = function () {
                window.location.href = <?php echo json_encode($back_url, JSON_UNESCAPED_SLASHES); ?>;
            };
